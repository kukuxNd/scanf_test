            ...
            switch (*f) {
                ...
                case 's': { // 字符串（支持 GB2312）
                    ...
                    if (!suppress) {
                        ...
                        // 如果用户显式提供了 size_t，则读取
                        va_list args_copy;
                        va_copy(args_copy, args);
                        int peek = va_arg(args_copy, int);
                        if (peek > 0 && peek < 1000000) { // 粗略判断是否为合理大小
...
int main() {
    ...
    // 测试 %f
    const char *input7 = "123.45 -0.001 1.23e-4";
    float f1, f2, f3;
    int result7 = my_sscanf(input7, "%f %f %f", &f1, &f2, &f3);
    printf("Test 7 - Parsed items: %d\n", result7);
    printf("f1: %f\n", f1);
    printf("f2: %f\n", f2);
    printf("f3: %f\n", f3);
